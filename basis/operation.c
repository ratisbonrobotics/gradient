...
static inline double op_gelu(double x, double _unused)
{
    (void)_unused;
    return 0.5 * x * (1 + tanh(sqrt(2 / M_PI) * (x + 0.044715 * pow(x, 3))));
}

static inline double op_derivative_gelu(double x, double grad)
{
...
struct operation_ gelu = {op_gelu, op_derivative_gelu, "gelu"};
